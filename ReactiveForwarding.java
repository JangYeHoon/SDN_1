...
public class ReactiveForwarding {
    ...
    @Property(name = "firstCheck", boolValue = false,
            label = "firstCheck Enable record metrics for reactive forwarding")
    private boolean firstCheck = false;

    @Property(name = "fifoCheck", intValue = 1,
            label = "Configure Flow Timeout for installed flow rules; " +
                    "default is 10 sec")
    private int fifoCheck = 1;

    @Property(name = "replaceCheck", intValue = 0,
            label = "Configure Flow Timeout for installed flow rules; " +
                    "default is 10 sec")
    private int replaceCheck = 0;
    ...
    HashMap<MacAddress, MacAddress> [][] flow_table = new HashMap[3][23];
    ArrayList<MacAddress>[][] switch_check = new ArrayList[3][23];
    private int [] mod_fail_count = new int[8];
    int [] long_check = new int[8];
    int [] short_check = new int[8];

    int [] video_index = new int[8];
    int [] web_index = new int[8];
    int packet_count = 0;
    int cnt = 0;
    ...
    public void activate(ComponentContext context) {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 23; j++) {
                flow_table[i][j] = new HashMap<>();
                switch_check[i][j] = new ArrayList<>();
            }
        }
        packet_count = 0;
        cnt = 0;
    ...
    private void readComponentConfiguration(ComponentContext context) {
        ...
        Boolean firstCheckoutEnabled =
                Tools.isPropertyEnabled(properties, "firstCheck");
        if (firstCheckoutEnabled == null) {
            log.info("Packet-out is not configured, " +
                             "using current value of {}", firstCheck);
        } else {
            firstCheck = firstCheckoutEnabled;
            log.info("Configured. Packet-out only forwarding is {}",
                     firstCheck ? "enabled" : "disabled");
        }
        ...
        fifoCheck = Tools.getIntegerProperty(properties, "fifoCheck", 1);
        log.info("Configured. Fifo Check is configured to {}", fifoCheck);

        replaceCheck = Tools.getIntegerProperty(properties, "replaceCheck", 0);
        log.info("Configured. Fifo Check is configured to {}", replaceCheck);
    }
    ...
    private class ReactivePacketProcessor implements PacketProcessor {
        ...
        public void process(PacketContext context) {
            ...
            if (pkt.receivedFrom().deviceId().toString().equals("of:0000000000000005") && firstCheck == true) {
                if (src_check.get(ethPkt.getSourceMAC().toString()) != null) {
                    int src_num = src_check.get(ethPkt.getSourceMAC().toString());
                    int [] num = new int[3];
                    int n = 0;
                    for (int i = 0; i < 3; i++) {
                        int index = switch_check[i][src_num].indexOf(ethPkt.getDestinationMAC());
                        try {
                            if (switch_check[i][src_num].get(index) != null) {
                                num[i] = 1;
                                switch_check[i][src_num].remove(index);
                            }
                    ...
                    if (num[2] == 1)
                        n += 1;
                    if (ethPkt.getSourceMAC().toString().equals("00:00:00:00:00:01") || ethPkt.getSourceMAC().toString().equals("00:00:00:00:00:02")
                            || ethPkt.getSourceMAC().toString().equals("00:00:00:00:00:03") || ethPkt.getSourceMAC().toString().equals("00:00:00:00:00:04")) {
                        video_index[n] += 1;
                    } else {
                        web_index[n] += 1;
                    }
...
//                log.info("cnt : " + cnt);
                if (cnt == 830343) {
                    ...
                    try {
                        if (replaceCheck == 0)
                            str = "fifo";
                        else if (replaceCheck == 1)
                            str = "lfu";
                        else if (replaceCheck == 2)
                            str = "lru";
                        else if (replaceCheck == 3)
                            str = "sff";
                        replaceCheck += 1;
                        for (int i = 0; i < 8; i++) {
                            FileWriter out_video, out_web;
                            String s;
                            if (video_index[i] != 0) {
                                out_video = new FileWriter("/home/sdn/switch/" + str + "/video/" + i + ".txt");
                                s = "count - " + video_index[i] + "\n";
                                out_video.write(s);
                                out_video.close();
                            }
                            if (web_index[i] != 0) {
                                out_web = new FileWriter("/home/sdn/switch/" + str + "/web/" + i + ".txt");
                                s = "count - " + web_index[i] + "\n";
                                out_web.write(s);
                                out_web.close();
                            }
                        ...
                        for (int i = 0; i < 3; i++) {
                            FileWriter output = new FileWriter("/home/sdn/count/" + str + "/count" + i + ".txt");
                            String s = System.currentTimeMillis() + "\n" + mod_fail_count[i] + "\n" + long_check[i] + "\n" + short_check[i] + "\ncnt : " + cnt + "\n";
                            output.write(s);
                            output.close();
                        }
                    } catch(Exception e) {e.getStackTrace();}
                    flowRuleService.setFlowFlag(null, null, null);
                    for (int i = 0; i < 8; i++) {
                        mod_fail_count[i] = 0;
                        long_check[i] = 0;
                        short_check[i] = 0;
                    ...
                    for (int i = 0; i < 3; i++) {
                        for (int j = 0; j < 23; j++) {
                            flow_table[i][j].clear();
                            switch_check[i][j].clear();
                        }
                    }
                    packet_count = 0;
                    cnt = 0;
    ...
    private void isMatchingPacket(Ethernet eth, DeviceId deviceId) {
        if (firstCheck == true) {
            if (eth.getSourceMAC() != null && eth.getDestinationMAC() != null) {
                if (src_check.get(eth.getSourceMAC().toString()) != null) {
                    if (fifoCheck == 0)
                        flowRuleService.updateIntervalTIme(eth.getSourceMAC(), eth.getDestinationMAC(), deviceId, 0);       // fifo == 1 , other == 0
                    else if (fifoCheck == 1)
                        flowRuleService.updateIntervalTIme(eth.getSourceMAC(), eth.getDestinationMAC(), deviceId, 1);       // fifo == 1 , other == 0
                    flowRuleService.setPacketCount(eth.getSourceMAC(), eth.getDestinationMAC(), deviceId);
                }
            }
        }
    }
    ...
    private void installRule(PacketContext context, PortNumber portNumber, ReactiveForwardMetrics macMetrics) {
        ...
        int switch_num = Integer.parseInt(pkt.receivedFrom().deviceId().toString().substring(pkt.receivedFrom().deviceId().toString().length() - 1)) - 2;
        if (firstCheck == true && switch_num >= 0 && switch_num <= 3) {
            Integer src_num;
            int flow_limit = 10;
            packetOut(context, portNumber, macMetrics);
            isMatchingPacket(inPkt, pkt.receivedFrom().deviceId());
            if (switch_num >= 0 && switch_num <= 2 && (src_num=src_check.get(inPkt.getSourceMAC().toString())) != null) {
                if (flow_table[switch_num][src_num].get(inPkt.getDestinationMAC()) == null) {
                    switch_check[switch_num][src_num].add(inPkt.getDestinationMAC());
                    int size = 0;
                    for (int i = 0; i < 23; i++) {
                        size += flow_table[switch_num][i].size();
                    }
                    if (switch_num == 1)
                        flow_limit = 30;
...
//                        flow_limit = 160;
                    ...
                    if (size >= flow_limit) {
                        replaceFlow r = new replaceFlow();
                        if (replaceCheck == 0)
                            r.fifo(src_num, switch_num, inPkt.getSourceMAC(), inPkt.getDestinationMAC(), pkt.receivedFrom().deviceId());
                        else if (replaceCheck == 1)
                            r.lfu(src_num, switch_num, inPkt.getSourceMAC(), inPkt.getDestinationMAC(), pkt.receivedFrom().deviceId());
                        else if (replaceCheck == 2)
                            r.lru(src_num, switch_num, inPkt.getSourceMAC(), inPkt.getDestinationMAC(), pkt.receivedFrom().deviceId());
                        else if (replaceCheck == 3)
                            r.sff(src_num, switch_num, inPkt.getSourceMAC(), inPkt.getDestinationMAC(), pkt.receivedFrom().deviceId());
                    }
                    flow_table[switch_num][src_num].put(inPkt.getDestinationMAC(), inPkt.getSourceMAC());
                }
            }
            return;
        }
    ...
    private class replaceFlow {
        ...
        int flag_check;
        boolean temp;
        MacAddress del_src, del_dst;

        public replaceFlow() {
            lru_value = 2000000000;
            lfu_value = 2000000000;
            fifo_value = 2000000000;
            sff_value = -200;
            flag_check = 0;
            del_dst = null;
            del_src = null;
        ...
        public void sff(int src_num, int switch_num, MacAddress src, MacAddress dst, DeviceId deviceId) {
            long currentTime = System.nanoTime();
                for (int i = 0; i < 23; i++) {
                    for (MacAddress d : flow_table[switch_num][i].keySet()) {
                        MacAddress s = flow_table[switch_num][i].get(d);
                        long interval_time = flowRuleService.getIntervalTime(s, d, deviceId, 1, currentTime);
                        int flow_flag = flowRuleService.getFlowFlag(s, d, deviceId);
                        if (temp == false) {
                            sff_value = interval_time;
                            flag_check = flow_flag;
                            del_src = s;
                            del_dst = d;
                            temp = true;
                        }
                        else if (flag_check == 0 && flow_flag == 1) {
                            continue;
                        }
                        else if (flag_check == 1 && flow_flag == 0) {
                            sff_value = interval_time;
                            flag_check = flow_flag;
                            del_src = s;
                            del_dst = d;
                        } else if (interval_time > sff_value) {
                            sff_value = interval_time;
                            flag_check = flow_flag;
                            del_src = s;
                            del_dst = d;
            ...
            del_save(src, dst, del_src, del_dst, switch_num, deviceId);
        }

        public void fifo(int src_num, int switch_num, MacAddress src, MacAddress dst, DeviceId deviceId) {
            long currentTime = System.nanoTime();
                for (int i = 0; i < 23; i++) {
                    for (MacAddress d : flow_table[switch_num][i].keySet()) {
                        MacAddress s = flow_table[switch_num][i].get(d);
                        long interval_time = flowRuleService.getIntervalTime(s, d, deviceId, 0, currentTime);
                        if (temp == false) {
                            fifo_value = interval_time;
                            del_src = s;
                            del_dst = d;
                            temp = true;
                        } else if (interval_time < fifo_value) {
                            fifo_value = interval_time;
                            del_src = s;
                            del_dst = d;
                        }
        ...
        public void lfu(int src_num, int switch_num, MacAddress src, MacAddress dst, DeviceId deviceId) {
            long currentTime = System.nanoTime();
                for (int i = 0; i < 23; i++) {
                    for (MacAddress d : flow_table[switch_num][i].keySet()) {
                        MacAddress s = flow_table[switch_num][i].get(d);
                        long interval_time = flowRuleService.getIntervalTime(s, d, deviceId, 0, currentTime);
                        int packet_count = flowRuleService.getPacketCount(s, d, deviceId);
                        if (temp == false) {
                            lfu_value = packet_count;
                            lru_value = interval_time;
                            temp = true;
                            del_src = s;
                            del_dst = d;
                        } else if (packet_count <= lfu_value) {
                            if (packet_count == lfu_value) {
                                if (interval_time < lru_value) {
                                    lfu_value = packet_count;
                                    lru_value = interval_time;
                                    del_src = s;
                                    del_dst = d;
        ...
        public void lru(int src_num, int switch_num, MacAddress src, MacAddress dst, DeviceId deviceId) {
            long currentTime = System.nanoTime();
                for (int i = 0; i < 23; i++) {
                    for (MacAddress d : flow_table[switch_num][i].keySet()) {
                        MacAddress s = flow_table[switch_num][i].get(d);
                        ...
                        if (temp == false) {
                            ...
                            del_src = s;
                            del_dst = d;
                        } else if (interval_time < lru_value) {
                            lru_value = interval_time;
                            del_src = s;
                            del_dst = d;
                        }
        ...
        private void del_save(MacAddress in_src, MacAddress in_dst, MacAddress del_src, MacAddress del_dst, int switch_num, DeviceId deviceId) {
            int num = src_check.get(del_src.toString());
            flow_table[switch_num][num].remove(del_dst);

            mod_fail_count[switch_num]++;
            if (del_src.toString().equals("00:00:00:00:00:01") || del_src.toString().equals("00:00:00:00:00:02")
                    || del_src.toString().equals("00:00:00:00:00:03") || del_src.toString().equals("00:00:00:00:00:04")) {
                long_check[switch_num]++;
            }
            else short_check[switch_num]++;

            flowRuleService.setZeroPacketCount(del_src, del_dst, deviceId);
        }
    }
    ...
    private class InternalFlowListener implements FlowRuleListener {
        ...
        public void event(FlowRuleEvent event) {
            ...
            if (event.type() == RULE_REMOVED && flowRule.appId() == appId.id() && firstCheck == true) {
                MacAddress src = null, dst = null;
                ...
                if (src != null && dst != null)
                    flowRuleService.setZeroPacketCount(src, dst, flowRule.deviceId());
            }
        }
    }
}